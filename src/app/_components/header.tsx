...
const Header = () => {
  ...
  return (
    <header>
      <div className="p-4 flex justify-between items-center max-w-6xl mx-auto text-accent-1">


        {/* Logo and title */}
        <h1 className="text-3xl font-bold hover:text-hover-accent-1">
          <Link href="/">
            <span className="cursor-pointer">UAANT</span>
          </Link>
        </h1>

        {/* Navigation Menus */}
        <nav className="hidden md:flex flex-grow items-center justify-center space-x-4 text-bold">
          {/* Desktop Navigation Links */}
          {MAIN_MENU.map((item, index) => (
            <Link href={item.link} key={index}>
              <span className="cursor-pointer hover:text-accent-2">{item.title}</span>
            </Link>
          ))}
        </nav>
